    ...
    try {
      ...
      if (!prompt) {
        if (simLeakType === "aws_keys") {
          ...
          prompt = "Analyzing turbine Mark VIe register 0x4001 at internal substation 192.168.1.50 with nominal grid frequency 60.2 Hz. What triggers sudden emergency trip?";
        } else if (simLeakType === "patient_ssn") {
          prompt = "Summarize clinical diagnosis for patient Johnathan Doe, SSN: 123-45-6789, MRN #98421 with acute cardiac arrhythmia.";
        } else if (simLeakType === "credit_card") {
          prompt = 'Verify charge webhook payload: { card: "4532-8812-9901-4321", cvv: "882", exp: "08/28", holder: "David R. Sterling" }';
        } else if (simLeakType === "jwt_secret") {
          prompt = 'Troubleshoot JWT token generation using master private secret: "jwt_secret_signing_key_prod_9942a" in our authorization middleware.';
        } else {
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="patient_ssn">Protected Health Info (Patient SSN + MRN)</option>
                    <option value="credit_card">PCI Credit Card (Visa 4532... + CVV)</option>
                    <option value="jwt_secret">Production API Signing Secret (JWT RS256)</option>
                  </select>
                </div>

                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: "var(--apple-text-muted)", marginBottom: 6, display: "block" }}>
                    Custom Prompt Payload (Optional)
                  </label>
                  <textarea
                    className="apple-input"
                    rows={2}
                    placeholder="Leave blank to use preconfigured realistic payload..."
